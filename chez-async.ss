;;; chez-async.ss - 统一入口
;;;
;;; 一行 (import (chez-async)) 即可用基于移植的 skiff C++ task 运行时的全部
;;; 高层 API：async/await 协程、fs、net、process、timer。
;;;
;;; 运行时是 native/ 的 libchez-async-rt.so（移植自 skiff src/runtime，纯
;;; C++/libuv）。import 本库即触发 load-shared-object，故运行需 native .so 在
;;; 路径上（LD_LIBRARY_PATH=native/<machine-type>，或已 `bake install`）。
;;;
;;; 用法：
;;;   (import (chez-async))
;;;   (io-runtime-start!)
